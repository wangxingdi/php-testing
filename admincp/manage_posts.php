...
<ol class="breadcrumb">
  ...
  <li class="active">Active Posts</li>
  <span class="theme-label">MarketPress v<?php echo $Settings['version'];?></span>
</ol>

<div class="page-header">
  <h3>Active Posts <small>Manage active posts</small></h3>
</div>
...
<section style="width: 100%;" class="col-md-8">
...
// How many adjacent pages should be shown on each side?
  ...
  $query = $mysqli->query("SELECT COUNT(*) as num FROM mp_posts WHERE active='1' ORDER BY id DESC");
  ...
  /* Get data. */
  $result = $mysqli->query("SELECT * FROM mp_posts WHERE active='1' ORDER BY id DESC LIMIT $start, $limit");
  ...
  $q= $mysqli->query("SELECT * FROM mp_posts WHERE active='1' ORDER BY id DESC limit $start,$limit");


  $numr = mysqli_num_rows($q);
  if ($numr==0)
  {
  echo '<div class="alert alert-danger">There are no active posts to display.</div>';
  }
  if ($numr>0)
  {
  ?>
...
?>        
        ...
        <td><center><a data-toggle="modal" href="preview.php?id=<?php echo $Row['id'];?>" data-target="#ProductModal"><img src="../cache/timthumb.php?src=http://<?php echo $SiteLink;?>/images/<?php echo $Row['image'];?>&amp;h=50&amp;w=50&amp;q=100" alt="<?php echo $LongTitle;?>" class="img-responsive"></a></center></td>
...
<div class="modal fade" id="DeactivateModal" tabindex="-1" role="dialog" aria-labelledby="DeactivateModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
         ...
         <p class="text-info"><small>You can re-activate this post by navigating to Blog > Deactivated Posts.</small></p> 
            </div>
            <!--/modal-body-collapse -->
            <div class="modal-footer">
