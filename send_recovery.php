...
//Get User Details
    if ($UserCheck = $mysqli->query("SELECT * FROM mp_users WHERE user_email='$EmailAddress'")) {
...
" . $FromName;
        ...
        if (!$mail->Send()) {
            ...
            $mysqli->query("UPDATE mp_users SET user_password='$EncryptPassword' WHERE user_id='$UserId'");
            ?>
            <div class="alert alert-success" role="alert">Your password has now been reset. Please check your email for
                your new login details.
            </div>
        <?php }
    } else { ?>
        <div class="alert alert-danger" role="alert">We cannot find any user with this email address. Please check your
            email address.
        </div>
    <?php }
}
