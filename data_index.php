...
$start = ($page - 1) * 15;
  if ($sort=="n"){
    $sortpage = "newest";
  $result = $mysqli->query("SELECT * FROM listings WHERE active=1 ORDER BY id DESC LIMIT $start, 15");
  }else if ($sort=="p"){
    $sortpage = "popular";
  $result = $mysqli->query("SELECT * FROM listings WHERE active=1 ORDER BY views DESC LIMIT $start, 15");
  }else if ($sort=="l"){
    $sortpage = "low";
  $result = $mysqli->query("SELECT * FROM listings WHERE active=1 ORDER BY CAST(price AS DECIMAL(10,2)) ASC LIMIT $start, 15"); 
  }else if ($sort=="h"){
    $sortpage = "high";
  $result = $mysqli->query("SELECT * FROM listings WHERE active=1 ORDER BY CAST(price AS DECIMAL(10,2)) DESC LIMIT $start, 15");
  }else{
    $sortpage = "none";
  $result = $mysqli->query("SELECT * FROM listings WHERE active=1 ORDER BY id DESC LIMIT $start, 15");
  }
