...
$start = ($page - 1) * 27;
  if ($sort=="n"){
    $sortpage = "newest";
  $result = $mysqli->query("SELECT * FROM listings WHERE catid IN (".$cat_str.") AND active=1 AND CAST(price AS UNSIGNED) BETWEEN '$min_new' AND '$max_new' ORDER BY id DESC LIMIT $start, 27"); 
  }else if ($sort=="p"){
    $sortpage = "popular";
  $result = $mysqli->query("SELECT * FROM listings WHERE catid IN (".$cat_str.") AND active=1 AND CAST(price AS UNSIGNED) BETWEEN '$min_new' AND '$max_new' ORDER BY views DESC LIMIT $start, 27");
  }else if ($sort=="l"){
    $sortpage = "low";
  $result = $mysqli->query("SELECT * FROM listings WHERE catid IN (".$cat_str.") AND active=1 AND CAST(price AS UNSIGNED) BETWEEN '$min_new' AND '$max_new' ORDER BY CAST(price AS DECIMAL(10,2)) ASC LIMIT $start, 27");
  }else if ($sort=="h"){
    $sortpage = "high";
  $result = $mysqli->query("SELECT * FROM listings WHERE catid IN (".$cat_str.") AND active=1 AND CAST(price AS UNSIGNED) BETWEEN '$min_new' AND '$max_new' ORDER BY CAST(price AS DECIMAL(10,2)) DESC LIMIT $start, 27");
  }else{
    $sortpage = "non";
  $result = $mysqli->query("SELECT * FROM listings WHERE catid IN (".$cat_str.") AND active=1 AND CAST(price AS UNSIGNED) BETWEEN '$min_new' AND '$max_new' ORDER BY id DESC LIMIT $start, 27");
  }
...
<a class="col-link col-link-data" href="offer_link.php?id=<?php echo $row['id'];?>" target="_blank">
...
<span class="info-saves"> &nbsp;<i class="fas fa-eye"></i>&nbsp;&nbsp;<?php echo $view_count; ?> views</span></div>
<div class="col-right"><a class="btn btn-default btn-warning pull-right btn-font btn-checkout" href="offer_link.php?id=<?php echo $row['id'];?>" target="_blank"><?php echo $settings['buy_button'];?></a></div></div><!-- /.col-bottom --></div><!-- /.col-box -->
<?php 
}?>
</div><!-- /.container -->
