...
if($cat_list_sql = $mysqli->query("SELECT * FROM categories"))
{
  while($cat_list_row = mysqli_fetch_array($cat_list_sql))
  {
   ...
   if($is_sub_cat != 1)
    { ?>
      <li><a class="<?php if ($Cat_Url == $cname){ echo 'active '; } ?>auto-localize" href="category/<?php echo $Cat_Url; ?>/"><?php echo $cat_name; ?></a></li> 
 <?php if($is_a_branch == 1)
      {
        if($sub_cat_list = $mysqli->query("SELECT * FROM categories WHERE parent_id = $cat_id"))
        {
          while($sub_cat_list_row = mysqli_fetch_array($sub_cat_list))
          {
              ...
              ?>
              <ul class="submenu">
                <li style="list-style: none;"><a class="<?php if ($sub_Cat_Url == $cname){ echo 'active '; } ?>auto-localize" href="category/<?php echo $sub_Cat_Url; ?>/"><i style="font-size: 11px;" class="fas fa-arrow-right"></i> <?php echo $sub_cat_name; ?></a></li>
              </ul>
  <?php   }
        }
        else
...
<div id="row" class="row">
  ...
  <div class="col-md-2 max small-screen-remove"><input type="text" name="max_price" id="max_price" class="form-control" value="<?php echo $max_price; ?>" disabled></div>
...
<script>$('#cat_sort').val('<?php echo $sort; ?>');var sort_value  = '<?php echo $sort; ?>';sort_value = sort_value.charCodeAt(0);$('#cat_sort').change(function(e){$('#cat_sort').attr('disabled', '');$('#cat_sort').removeAttr('disabled');e.preventDefault();sort_value = $('#cat_sort').val().charCodeAt(0);});$(document).ready(function(){$( "#price_range" ).slider({range: true,min: 0,max: '<?php echo $max_price; ?>',values: [ <?php echo $minimum_range; ?>, <?php echo $max_price; ?>],change:function(event, ui, cid, sort_value){$("#minimum_range").val(ui.values[0]);$("#max_price").val(ui.values[1]);load_product(ui.values[0], ui.values[1], <?php echo $cid; ?>, sort_value);}});load_product(<?php echo $minimum_range; ?>, <?php echo $max_price; ?>, <?php echo $cid; ?>, sort_value);function load_product(minimum_range, max_price, cid, sort){$.ajax({url:"fetch_products.php",method:"POST",data:{minimum_range:minimum_range, max_price:max_price, cid:cid, sort:sort_value},success:function(data){$('#display-posts').html(data);}});}});</script>
<script type="text/javascript">var sort="<?php echo $sort; ?>";sort=sort.charCodeAt(0);var cat_url="<?php echo $CategoryUrl; ?>",pageurl="";$("#cat_sort").change(function(r){$("#cat_sort").attr("disabled",""),$("#cat_sort").removeAttr("disabled"),r.preventDefault();var a=$("#minimum_range").val(),t=$("#max_price").val(),e=$("#cid").val();switch($("#cat_sort").val()){case"newest":sort=110,pageurl="category/"+cat_url+"/";break;case"popular":sort=112,pageurl="category/"+cat_url+"/popular/";break;case"high":sort=104,pageurl="category/"+cat_url+"/high/";break;case"low":sort=108,pageurl="category/"+cat_url+"/low/";break;default:sort=110,pageurl="category/"+cat_url+"/"}$.ajax({url:"fetch_products.php",method:"POST",data:{minimum_range:a,max_price:t,cid:e,sort:sort},success:function(r){$("#display-posts").html(r),window.history.pushState("object or string","Title",pageurl)}})});0==$("#minimum_range").val()&&0==$("#max_price").val()&&($("#row").hide(),$("#display-posts").css("margin-top","0"));</script>