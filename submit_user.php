...
if ($_POST) {
    ...
    if ($UserCheck = $mysqli->query("SELECT * FROM mp_users WHERE user_name ='$UN'")) {
        $VdUser = mysqli_fetch_array($UserCheck);
        $UNV = $VdUser['user_name'];
        $UserCheck->close();
    } else {
    ...
    if ($EmailCheck = $mysqli->query("SELECT * FROM mp_users WHERE user_email ='$email_address'")) {
        $EmailRow = mysqli_fetch_array($EmailCheck);
        $ValidateEmail = $EmailRow['user_email'];
        $EmailCheck->close();
    } else {
        printf("<div class='alert alert-danger alert-pull'>There seems to be an issue. Please Trey again</div>");
    }
...
// Insert info into database table.. do w.e!
    $mysqli->query("INSERT INTO mp_users(user_name, user_email, user_password, user_register_date) VALUES ('$UserName', '$Email', '$EnPassword','$RegDate')");
    ?>
